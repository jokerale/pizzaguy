...
dests = np.array(dfNodi[0])
print(dests)

dests_sort = np.append(dests, 0) # add pizzeria node
dests_sort = np.sort(dests_sort)


df = pd.read_csv('data/' + str(input_var.city[0]) + '_dijkstra.csv',index_col=False)

df_r = df[[str(d) for d in dests_sort]]
df_r = df_r.loc[dests_sort]
...
stringaDist = dataStringGenerator(df_r, "mdist")

dest2write = [np.where(dests == int(c))[0][0] + 1 + 1 for c in df_r.drop(columns='0').columns]

stringaOrari = dataStringGenerator(dfOrari, "orario")
stringaPizze = dataStringGenerator(dfPizze, "num_pizze")
stringaNodi = dataStringGenerator(pd.DataFrame(dest2write), "dest")

# push random data on a temporary file
file = open('data/temp-order-cheat.dzn', 'w')
file.write(stringaDist + os.linesep
           + stringaOrari + os.linesep 
           + stringaPizze + os.linesep 
           + stringaNodi+ os.linesep
           + "N = " + str(N) + ";" + os.linesep
           + "k = " + str(new_k) + ";" + os.linesep
           + "d = " + str(d) + ";" + os.linesep)
file.close()
...
cmd = 'minizinc --solver Gecode --time-limit ' + str(timelimit) + ' v04-cheat.mzn data/temp-order-cheat.dzn --statistics'  
